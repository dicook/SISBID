...
- two new variables are introduced (in key-value format): **key** and **value**: multiple columns are collapsed; their names go into variable `key`, their values go into variable `value`.
...
- `sep` is the splitting separator. By default splits happen for any non-alphanumeric value.
- better specify `sep` to have some more control; any regular expression will work
...
```{r}
tb2 <- tb1 %>% separate(key, sep = "_", into=c("foo_new", "foo_sp", "genderage")) 
tb2 %>% na.omit() %>% head()
```
...
How do ratigs change over the weeks?
...
gtidy %>% 
  filter(time==6 ) %>%
  ggplot(aes(x = trt, y = expr, colour=id)) +
  geom_point() +
