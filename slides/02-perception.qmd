...
tb_kn |> filter(year == 2012) |>
  ...
  ggtitle("Arrangement A") + guides(fill = "none")
```
...
tb_kn |> filter(year == 2012) |>
  ...
  ggtitle("Arrangement B") + guides(fill="none")
```
:::
...
Go to www.menti.com and use the code 6630 2740. 
...
ggplot(tb_kn, aes(x=year, y=count, colour=sex)) +
  ...
  ggtitle("Type A") + guides(color = "none")
```
...
ggplot(tb_kn, aes(x=year, y=count, fill=sex)) +
  ...
  facet_wrap(~age, ncol=6) +
  scale_fill_manual("Sex", values = c("#DC3220", "#005AB5")) +
  ggtitle("Type B") + guides(fill = "none")
```
:::
...
tb_kn |> group_by(year, age) |> 
  ...
  geom_hline(yintercept = 0.50, colour="grey70", linewidth=2) +
  geom_line() + geom_point() +
  facet_wrap(~age, ncol=6) +
...
- Wagemans et al. (2012) A Century of Gestalt Psychology in Visual Perception: 
  - [I. Perceptual Grouping and Figure-Ground Organization](http://dx.doi.org/10.1037/a0029333)
  - [II. Conceptual and Theoretical Foundations](https://doi.org/10.1037/a0029334)
