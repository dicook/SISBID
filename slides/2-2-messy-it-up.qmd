...
## Your turn: What is the correlation between scales? {.inverse}
...
## Your turn: Modeling ratings over time & scales? {.inverse .middle}
...
```{r ratings by week again, echo=FALSE, fig.width=8, fig.height=3, message = F, warning = F}
ff.m$time <- as.numeric(ff.m$time)
ggplot(data=ff.m, aes(x=time, y=rating, colour=type)) + 
geom_point(size=.75) +
geom_smooth() +
