...
<<<<<<< HEAD:slides/2-6-adv-graphics.qmd
=======
...
>>>>>>> 4878b57bfe0f02cf286f5e601203113922ee28c5:slides/2.6-adv-graphics/index.Rmd
---
...
![](images/drob_twitter.png){width="300px"}

Based on [an analysis in Tick Tock blog](https://ticktocksaythehandsoftheclock.wordpress.com/2018/01/11/capitals-and-good-governance/) by Graham Tierney
...
```{r, eval = F}
ggplot(data) + geom_point(aes(x=x1, y=x2)) + ggtitle("A")

ggplot(data) + geom_point(aes(x=x1, y=x2, colour=cl)) + ggtitle("B")

ggplot(data) + geom_histogram(aes(x=x1))+ ggtitle("C")

ggplot(data) + geom_boxplot(aes(x=cl, y=x1))+ ggtitle("D")
```

[Which plot definition would most match to a null hypothesis stating **there is no difference in the distribution between the groups?**]{.orange}
...
B. $H_o:$ no difference between levels of `cl`
C. $H_o:$ the distribution of `x1` is XXX
D. $H_o:$ no difference in the distribution of `x1` between levels of `cl`

## Let's do it

::: columns
::: column
![](images/nullabor_hex.png){width="20%"}
...
## Your turn {background-image="images/cookie-monster.jpg" background-size="12%" background-position = "100% 0%"}
