...
```{r lineup 1, fig.height=6, fig.width=6, echo=T}
...
```{r embed the data plot in a field of null plots, eval = F}
pos <- sample(1:20, 1)
df_null <- lineup(
  null_permute('v1'), 
  df, pos=pos)
...
```{r penguins-lineup-ex, echo = F, message = F, warning = F}
set.seed(203482093)
data(penguins)
pos <- sample(1:20, 1)
...
```{r lineup 2, fig.height=6, message = F, warning = F, eval=F}
...
```{r lineup 2, fig.height=6, message = F, warning = F, echo= F}
```
...
```{r lets talk TB, echo=FALSE,  message = F, warning = F}
...
```{r tb-lineup-1, fig.height=6, fig.width=10, echo = F}
```
...
```{r a more complicated null, echo=TRUE, code_copy=F}
...
```{r tb-lineup-2, fig.height=6, fig.width=10, echo = F}
```
...
```{r make a map lineup, fig.height=6, fig.width=16, fig.cap = "Cancer incidence across the US 2010-2014, Melanoma cases per 100k. [Data source: American Cancer Society](https://cancerstatisticscenter.cancer.org)."}
