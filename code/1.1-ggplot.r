...
tb_us %>%
  ggplot() +
  geom_line(aes(year, count, col = age_group), size = 0.7) +
  facet_grid(~sex) +
  theme_light()

tb_us %>%
  ggplot() +
  geom_point(aes(year, count, col = age_group), size = 0.5) +
  geom_smooth(aes(year, count, col = age_group), method = "lm", se = F, size = 0.7) +
  facet_grid(~sex) +
  theme_light()
