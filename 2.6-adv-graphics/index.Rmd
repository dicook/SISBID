...
```{r emake a choropleth map, cho=TRUE}
...
```{r compute the linear trend in counts, echo=TRUE, eval=FALSE}
# This is  code to compute the linear trend in counts
# for each country.
...
```{r one way to make the map is use geom_map, echo=TRUE, eval=FALSE}
...
```{r aother approach is to join the polygon data, echo=TRUE, eval=FALSE}
...
```{r why is India showing a drastically increasing TB incidence, echo=FALSE, eval=FALSE}
tb_india <- tb %>% 
...
```{r solution to hy is India showing a drastically increasing TB incidence, fig.height=7, echo=FALSE, eval=FALSE}
ggplot(tb_map_india) + 
...
```{r interactive plot solution, echo=FALSE, eval=FALSE}
