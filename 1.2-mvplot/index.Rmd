...
Make a scatterplot matrix of the the states against each other.

```{r eval=FALSE}
sm_state_count <- salmonella %>% 
  mutate(week = week(date), year = year(date)) %>%
  count(State, week) %>%
  filter(week < 53, week > 1) %>% 
  spread(State, n) 
ggscatmat(sm_state_count[,-1])
```
...
Conditioning provides alternative ways to understand relationships. 
